...
try {
    ...
    ");
    $stmt->execute([$id_produit, $id_producteur]);
    $produit = $stmt->fetch();
    
    if (!$produit) {
        header("Location: dashboard_producteur.php?error=produit_non_trouve");
        exit();
    }
            ...
            if (empty($error)) {
                ...
                $stmt = $pdo->prepare("
                    ...
                    SET id_categorie = ?, nom_produit = ?, description = ?, 
                        prix_unitaire = ?, stock_quantite = ?, statut_publie = ?, photo_url = ?
                    WHERE id_produit = ?
                ");
                $stmt->execute([
                    $id_categorie,
                ...
                header("Location: gerer-boutique.php?id=" . $produit['id_boutique'] . "&product_updated=1");
                exit();
            }
        } catch(PDOException $e) {
...
<head>
    ...
    <title>Modifier le produit - GreenMarket</title>
...
<div class="container">
    ...
    <a href="gerer-boutique.php?id=<?php echo $produit['id_boutique']; ?>" class="btn-back">
    ...
    <div class="form-card">
        <div class="boutique-info">
            <span class="name"><i class="bi bi-shop"></i> <?php echo htmlspecialchars($produit['nom_boutique']); ?></span>
            <span class="badge">Modification du produit</span>
        </div>

        <h2 style="font-family:'Playfair Display',serif;font-size:1.2rem;color:var(--primary);margin-bottom:1.5rem;">
            <i class="bi bi-box-seam"></i> <?php echo htmlspecialchars($produit['nom_produit']); ?>
        </h2>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nom_produit" 
                       value="<?php echo htmlspecialchars($produit['nom_produit']); ?>"
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="prix_unitaire" 
                           step="0.01" min="0.01"
                           value="<?php echo htmlspecialchars($produit['prix_unitaire']); ?>"
                           required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="stock_quantite" 
                           min="0"
                           value="<?php echo htmlspecialchars($produit['stock_quantite']); ?>"
                           required>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="current-image">
                    <img src="<?php echo htmlspecialchars($produit['photo_url']); ?>" alt="Image du produit">
                </div>
            ...
            <div class="form-group">
                ...
                <select name="statut_publie">
                    <option value="Brouillon" <?php echo $produit['statut_publie'] == 'Brouillon' ? 'selected' : ''; ?>>Brouillon (non visible)</option>
                    <option value="Publié" <?php echo $produit['statut_publie'] == 'Publié' ? 'selected' : ''; ?>>Publié (visible)</option>
                    <option value="Suspendu" <?php echo $produit['statut_publie'] == 'Suspendu' ? 'selected' : ''; ?>>Suspendu</option>
                </select>
                <div class="help-text">
